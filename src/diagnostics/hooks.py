...
def maybe_register_inf_check_hooks(model: torch.nn.Module) -> None:
    """Conditionally register inf/nan check hooks based on the
    environment variable ``INF_CHECK``.

    Args:
      model:
        the model to be analyzed.
    """
    if not os.environ.get("INF_CHECK", ""):
        return
